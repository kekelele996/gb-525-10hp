...
import { measureTypeLabels, mitigationLabels } from '@/types/mitigation'
...
<template>
  <section class="evidence-panel" aria-live="polite">
    ...
    <template v-else>
      ...
      <div v-if="item.mitigation" class="mitigation-block">
        <ShieldCheck :size="16" /><div class="mitigation-copy"><div><strong>{{ measureTypeLabels[item.mitigation.measure_type] }}措施 · {{ mitigationLabels[item.mitigation.status] }}</strong><span>完成日期 {{ item.mitigation.completed_on }}</span></div><p>{{ item.mitigation.evidence_note }}</p></div>
      </div>
    </template>
  </section>
</template>
