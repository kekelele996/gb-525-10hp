...
import { ArrowRight, Beaker, ShieldCheck, Wrench } from 'lucide-vue-next'
import RiskBadge from './RiskBadge.vue'
import { measureTypeLabels } from '@/types/mitigation'
...
<template>
  <section class="evidence-panel" aria-live="polite">
    ...
    <template v-else>
      ...
      <div v-if="item.mitigation" class="mitigation-band">
        <ShieldCheck :size="17" /><div class="mitigation-copy"><div><strong>已缓解 · {{ measureTypeLabels[item.mitigation.measure_type] }}</strong><span>措施 #{{ item.mitigation.measure_id }} · 完成于 {{ item.mitigation.completed_at }} · 路线 v{{ item.mitigation.route_version }} · 复核 {{ dateTime(item.mitigation.reviewed_at) }}</span></div><p>{{ item.mitigation.evidence_note }}</p></div>
      </div>
      <div class="edge-list">
