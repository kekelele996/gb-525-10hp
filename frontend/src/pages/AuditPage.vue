...
<template>
  ...
  <section class="content-band">
    <div class="toolbar"><el-select v-model="query.entity_type" clearable placeholder="全部实体" style="width: 170px" @change="load"><el-option label="过敏原谱" value="allergen_profile" /><el-option label="工艺路线" value="process_route" /><el-option label="接触边" value="contact_edge" /><el-option label="评估" value="assessment_run" /><el-option label="缓解措施" value="mitigation_measure" /></el-select><el-input v-model="query.action" clearable placeholder="动作代码" :prefix-icon="Search" style="width: 210px" @keyup.enter="load" /><el-input v-model="query.request_id" clearable placeholder="Request ID" class="request-search" @keyup.enter="load" /><el-tooltip content="刷新"><el-button :icon="RefreshCw" circle aria-label="刷新" @click="load" /></el-tooltip><span class="toolbar-spacer subtle-count">{{ total }} 条事件</span></div>
