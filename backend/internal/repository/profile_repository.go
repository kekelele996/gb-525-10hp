...
func (r *profileRepository) Update(ctx context.Context, profile *model.AllergenProfile, expected uint, scope AuditContext) error {
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		invalidated, err := invalidateAllMeasures(tx)
		if err != nil {
			return err
		}
		...
		audit, err := makeAudit(scope, "profile.versioned", "allergen_profile", profile.ID, profileSummary(before), profileSummary(*profile), map[string]any{"from_version": expected, "to_version": profile.Version, "assessments_staled": true, "measures_invalidated": invalidated})
		if err != nil {
			return err
		}
		if err := tx.Create(&audit).Error; err != nil {
