...
func (r *routeRepository) Update(ctx context.Context, route *model.ProcessRoute, expected uint, scope AuditContext) error {
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		invalidated, err := invalidateRouteMeasures(tx, route.ID)
		if err != nil {
			return err
		}
		...
		audit, err := makeAudit(scope, "route.versioned", "process_route", route.ID, routeSummary(before), routeSummary(*route), map[string]any{"from_version": expected, "to_version": route.Version, "assessments_staled": true, "measures_invalidated": invalidated})
		if err != nil {
			return err
		}
		if err := tx.Create(&audit).Error; err != nil {
