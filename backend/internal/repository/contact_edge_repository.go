...
func (r *contactEdgeRepository) Create(ctx context.Context, edge *model.ContactEdge, scope AuditContext) error {
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		invalidated, err := markRouteMitigationsInvalidated(tx, edge.RouteID)
		if err != nil {
			return err
		}
		audit, err := makeAudit(scope, "contact_edge.created", "contact_edge", edge.ID, "", edgeSummary(*edge), map[string]any{"route_id": edge.RouteID, "version": edge.Version, "assessments_staled": true, "mitigations_invalidated": invalidated})
		if err != nil {
			return err
		}
...
func (r *contactEdgeRepository) Update(ctx context.Context, edge *model.ContactEdge, expected uint, scope AuditContext) error {
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		invalidated, err := markRouteMitigationsInvalidated(tx, before.RouteID)
		if err != nil {
			return err
		}
		...
		audit, err := makeAudit(scope, "contact_edge.versioned", "contact_edge", edge.ID, edgeSummary(before), edgeSummary(*edge), map[string]any{"route_id": edge.RouteID, "from_version": expected, "to_version": edge.Version, "assessments_staled": true, "mitigations_invalidated": invalidated})
		if err != nil {
			return err
		}
		if err := tx.Create(&audit).Error; err != nil {
