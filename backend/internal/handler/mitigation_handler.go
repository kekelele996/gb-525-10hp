...
func (h *MitigationHandler) Resubmit(c *gin.Context) {
	id, ok := util.PathID(c, "id")
	if !ok {
		return
	}
	...
	var request dto.ResubmitMitigationRequest
	if !util.BindJSON(c, &request, h.validate) {
		return
	}
	item, err := h.service.Resubmit(c.Request.Context(), id, request, principal, RequestID(c))
	if err != nil {
		util.Error(c, err)
		return
	}
